...
public class CPU extends Player {
    ...
    public void play() throws GameException {
        ...
        boolean hasMarbleOnTrack = !actionableMarbles.isEmpty();
        ...
        // If no marbles on track and no King/Ace, discard a card
        if (!hasMarbleOnTrack && !hasFieldCard) {
            // Discard a random card from hand
            if (!cards.isEmpty()) {
                int idx = (int) (Math.random() * cards.size());
                Card toDiscard = cards.get(idx);
                this.getHand().remove(toDiscard);
                // Optionally, add to firepit if accessible
            }
            return;
        }
